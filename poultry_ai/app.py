...
async def analyze_video(video: UploadFile = File(...)):
    ...
    while cap.isOpened():
        ...
        if results.boxes.id is not None:
            for box, track_id in zip(results.boxes, results.boxes.id):
                ...
                # Unique count (no double counting)
                if track_id not in seen_track_ids:
                    seen_track_ids.add(track_id)
                    class_counts[class_name] = class_counts.get(class_name, 0) + 1

                # ==============================
                # STEP 6: COLLECT BIRD AREA
                # ==============================
                if class_name == "bird":
                    area = (x2 - x1) * (y2 - y1)
    ...
    # ==============================
    # STEP 6: WEIGHT PROXY CALCULATION
    # ==============================
    avg_area_per_bird = {
        tid: sum(areas) / len(areas)
        for tid, areas in bird_areas.items()
    }
    ...
    weight_index = {
        str(tid): round(area / global_avg_area, 2)
        for tid, area in avg_area_per_bird.items()
    }

    # ==============================
    # STEP 7: FINAL COMPLIANT JSON
    # ==============================
    return {
        ...
        "tracks_sample": [
            {
                "track_id": tid,
                "weight_index": weight_index.get(tid)
            }
